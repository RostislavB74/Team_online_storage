...
from discounts.models import PersonalDiscount, BirthdayDiscount, Coupon, BonusAccount
...
class BirthdayDiscountInline(admin.TabularInline):
    ...
    extra = 0
    verbose_name = _("Знижка до Дня народження")
    verbose_name_plural = _("Знижки до Дня народження")
...
class BonusAccountInline(admin.StackedInline):
    ...
    verbose_name_plural = _("Бонусний рахунок")
...
class UserProfileAdminForm(forms.ModelForm):
    ...
    def clean_messengers(self):
        ...
        if not isinstance(messengers, dict):
            raise forms.ValidationError(_("Messengers must be a dictionary"))

        allowed_messengers = set(settings.ALLOWED_MESSENGERS)
        for key in messengers:
            if key not in allowed_messengers:
                raise forms.ValidationError(_("Unsupported messenger: {}".format(key)))
            ...
            if key in {"viber", "whatsapp"} and messengers[key]:
                if not re.match(r"^\+?\d{10,15}$", messengers[key]):
                    raise forms.ValidationError(
                        _(
                            "Invalid {} format. Must be a phone number (e.g., +380123456789)"
                        ).format(key)
                    )
            ...
            if key == "telegram" and messengers[key]:
                if not (
                    messengers[key].startswith("@")
                    or re.match(r"^\+?\d{10,15}$", messengers[key])
                ):
                    raise forms.ValidationError(
                        _(
                            "Telegram must start with @ (e.g., @username) or be a phone number (e.g., +380123456789)"
                        )
                    )

            # Перевірка для signal (не може бути порожнім)
            if key == "signal" and messengers[key] == "":
                raise forms.ValidationError(_("Signal ID cannot be empty"))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and hasattr(self.instance, "_messengers"):
            ...
            self.initial["messengers"] = (
                self.instance._messengers if self.instance._messengers else {}
            )
            if "messengers" in self.data:
                try:
                    self.initial["messengers"] = json.loads(
                        self.data.get("messengers", "{}")
                    )
...
class UserProfileAdmin(admin.ModelAdmin):
    ...
    get_messengers_display.short_description = _("Messengers")

    def get_orders_display(self, obj):
        if not obj.user:
            return _("No user associated")
        orders = obj.user.orders.all()
        if not orders:
            return _("No orders")
        return ", ".join([f"Order #{order.id} ({order.status})" for order in orders])

    get_orders_display.short_description = _("Orders")
    ...
    orders_display.short_description = _("User Orders")
