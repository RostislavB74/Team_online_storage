...
from .models import Order, OrderItem, OrderStock
from discounts.models import Coupon
from product.models import SubProducts
from product.utils import get_discounted_price
from decimal import Decimal

# Серіалізатор для OrderStock
class OrderStockSerializer(serializers.ModelSerializer):
    warehouse = serializers.SerializerMethodField()

    class Meta:
        model = OrderStock
        fields = ['warehouse', 'quantity', 'in_transit']
    @extend_schema_field(str)
    def get_warehouse(self, obj):
        return obj.warehouse_stock.warehouse.name if obj.warehouse_stock else None

# Серіалізатори з попереднього запиту
...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    @extend_schema_field({
        ...
        'properties': {
            ...
            'ean_13': {'type': 'string', 'nullable': True},
            'sku': {'type': 'string'},
            'article': {'type': 'string'},
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    stocks = OrderStockSerializer(many=True, read_only=True)
