...
class Order(TranslatableModel):
    ...
    birthday_discount = models.ForeignKey(
        "discounts.BirthdayDiscount", on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    def select_warehouse(self, product, quantity, address):
        ...
        if not address:
            ...
            address_city = (
                address.split(",")[0].strip().lower()
            )  # Беремо першу частину адреси (місто)
            warehouses = sorted(
            ...
            try:
                ...
                if warehouse_stock.quantity >= quantity:
                    return (
                        warehouse_stock,
                        False,
                    )  # Товар є на складі, переміщення не потрібне
            except WarehouseStock.DoesNotExist:
                continue
            # Якщо товару немає, можна позначити як in_transit або кинути виняток
            print(
                f"No stock available for product {product.name}. Awaiting replenishment."
            )
        ...
        for warehouse in Warehouse.objects.all():
            try:
                ...
                if warehouse_stock.quantity >= quantity:
                    print(
                        f"Product {product.name} will be transferred from {warehouse.name} to {address}"
                    )
        ...
        raise ValueError(
            f"Not enough stock for product {product.name} on any warehouse"
        )
    ...
    def _deduct_inventory(self):
        ...
        for item in self.items.all():
            ...
            warehouse_stock, in_transit = self.select_warehouse(
                product, item.quantity, self.address
            )

            # Списуємо товар
            if warehouse_stock.quantity < item.quantity:
                raise ValueError(
                    f"Insufficient stock for product {product.name} in warehouse {warehouse_stock.warehouse.name}"
                )
    ...
    def _mark_shipped(self):
        ...
        if self.delivery_method == "pickup":
            warehouses = {
                stock.warehouse_stock.warehouse.name for stock in self.stocks.all()
            }
            print(
                f"Order #{self.id} ready for pickup by {self.recipient_name} at {', '.join(warehouses)}"
            )
        else:
            total_weight = sum(
            ...
            if in_transit:
                print(
                    f"Order #{self.id} is in transit and will be shipped to {self.address}, total weight: {total_weight} kg"
                )
            else:
                print(
                    f"Order #{self.id} has been shipped to {self.address}, total weight: {total_weight} kg"
                )
...
class OrderStock(models.Model):
    ...
    warehouse_stock = models.ForeignKey(
        "warehouse.WarehouseStock", on_delete=models.CASCADE
    )
    quantity = models.PositiveIntegerField()
